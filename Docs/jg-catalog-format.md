...
- **System namespace** (`0xFExxxx`): well-known fields.
  ...
  - **Row state** — four flags the router computes and sends on table after table, which **no `.jg`
    window declares at all**, so nothing in this format names them:

    | Key | API spelling | Meaning |
    |---|---|---|
    | `0xFE0007` | `dynamic` | learned or generated by the router, not configured |
    | `0xFE0008` | `invalid`, `inactive` on `/interface` | configured but not in effect |
    | `0xFE000D` | `default`, `builtin` on `/interface/list` | ships with the router |
    | `0xFE000A` | `disabled` | the enable/disable toggle (`types.enable` writes it) |
