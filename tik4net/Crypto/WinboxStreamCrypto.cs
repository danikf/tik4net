...
// Shared by MAC-Telnet auth and WinBox MAC/TCP encryption.

using System;
using System.Linq;
using System.Security.Cryptography;

namespace tik4net.Crypto
{
    /// <summary>
    /// Derives AES-128 + HMAC-SHA1 stream keys from an EC-SRP5 shared secret,
    /// and provides AES-128-CBC MAC-then-Encrypt encrypt/decrypt helpers (WinBox M2 framing).
    /// </summary>
    internal static class WinboxStreamCrypto
    {
        ...
        {
            byte[] m2 = System.Text.Encoding.ASCII.GetBytes(
                "On the client side, this is the send key; on the server side, it is the receive key.");
            byte[] m3 = System.Text.Encoding.ASCII.GetBytes(
