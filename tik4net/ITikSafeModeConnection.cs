...
namespace tik4net
{
    ...
    /// </summary>
    /// <remarks>
    /// Split off <see cref="ITikConnection"/> in 5.0, because safe mode needs a persistent, session-bound
    ...
    /// <summary>
    /// Reaches <see cref="ITikSafeModeConnection"/> from a plain <see cref="ITikConnection"/>. A transport
    /// that cannot bind safe mode to a session (REST) throws the same capability error it threw when these
    /// methods still lived on the connection interface.
    /// </summary>
    public static class TikSafeModeExtensions
    {
        ...
        /// <inheritdoc cref="ITikSafeModeConnection.SafeModeTake"/>
        /// <exception cref="TikConnectionCapabilityNotSupportedException">The transport cannot bind safe mode to the connection.</exception>
        public static void SafeModeTake(this ITikConnection connection)
            => AsSafeModeConnection(connection).SafeModeTake();

        /// <inheritdoc cref="ITikSafeModeConnection.SafeModeRelease"/>
        /// <exception cref="TikConnectionCapabilityNotSupportedException">The transport cannot bind safe mode to the connection.</exception>
        public static void SafeModeRelease(this ITikConnection connection)
            => AsSafeModeConnection(connection).SafeModeRelease();

        /// <inheritdoc cref="ITikSafeModeConnection.SafeModeUnroll"/>
        /// <exception cref="TikConnectionCapabilityNotSupportedException">The transport cannot bind safe mode to the connection.</exception>
        public static void SafeModeUnroll(this ITikConnection connection)
            => AsSafeModeConnection(connection).SafeModeUnroll();

        /// <inheritdoc cref="ITikSafeModeConnection.SafeModeGet"/>
        /// <remarks>
        /// Answers <c>false</c> — rather than throwing — on a transport that has no safe mode: "does this
        /// connection hold safe mode" has a truthful answer there, and a <c>finally</c> block asking it
        /// must not fail on the way out.
        /// </remarks>
        public static bool SafeModeGet(this ITikConnection connection)
        {
            Guard.ArgumentNotNull(connection, nameof(connection));
            return connection is ITikSafeModeConnection safeMode && safeMode.SafeModeGet();
        }

        /// <summary>
        /// The connection as an <see cref="ITikSafeModeConnection"/>, or a capability error naming what is
        /// missing.
        /// </summary>
        public static ITikSafeModeConnection AsSafeModeConnection(this ITikConnection connection)
        {
            Guard.ArgumentNotNull(connection, nameof(connection));
            if (connection is ITikSafeModeConnection safeMode)
                return safeMode;
            throw new TikConnectionCapabilityNotSupportedException(TikConnectionCapability.SafeMode, Unsupported);
        }
    }
}
