namespace tik4net.Winbox
{
    ...
    internal static class WinboxM2Protocol
    {
        ...
        internal static class RecordKey
        {
            ...
            /// <summary>
            /// Dynamic — well-known read-only flag: the row was learned or generated by the router rather
            /// than configured. The API prints it as <c>dynamic</c>.
            /// </summary>
            /// <remarks>
            /// Confirmed on 7.24 by moving it, on one path with one variable: a <c>/ip/arp</c> table holding
            /// two learned rows and one added by hand answers <c>0xFE0007</c> True, True, False where the API
            /// says <c>dynamic=true,true,false</c>, with every other key of the three rows identical. No
            /// <c>.jg</c> window in the 7.24 catalog declares a field labelled 'Dynamic', which is why the
            /// flag reached no caller before.
            /// </remarks>
            internal const int Dynamic = 0xFE0007;
            /// <summary>
            /// Invalid — well-known read-only flag: the row is configured but not in effect. The API prints
            /// it as <c>invalid</c> on a config table and as <c>inactive</c> on <c>/interface</c>.
            /// </summary>
            /// <remarks>
            /// Confirmed on 7.24 by moving it: an <c>/ip/address</c> added to an interface that was then
            /// disabled answers <c>0xFE0008</c> True where the row beside it answers False, matching the
            /// API's <c>invalid=true</c>/<c>false</c> on the same two rows. The same read re-confirmed
            /// <see cref="Dynamic"/> on a second table.
            /// </remarks>
            internal const int Invalid = 0xFE0008;
            ...
            /// <summary>
            /// Builtin — well-known read-only flag: the row ships with the router and is not user-created.
            /// The API prints it as <c>builtin</c>.
            /// </summary>
            /// <remarks>
            /// <para>Confirmed on 7.24 the same way <see cref="Dynamic"/> is: <c>/interface/list</c> answers
            /// True for its four shipped lists and False for one created for the test, matching the API's
            /// <c>builtin</c> exactly.</para>
            /// <para>Shares its number with <see cref="Command.GetSingleton"/> and
            /// <see cref="Error.Timeout"/>, which live in the command and error namespaces — the overlap the
            /// class doc describes, not a collision: this one is a field key inside a record.</para>
            /// </remarks>
            internal const int Builtin = 0xFE000D;
